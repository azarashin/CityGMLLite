...
データベースのER図と日本語データ辞書は、[SQLiteデータベース定義](database-schema.md)および[SQLiteデータ辞書](data-dictionary.md)を参照してください。現在のスキーマは建物（`Building` / `BuildingPart`）を対象とし、地形・水部・道路などの共通地物はまだ定義していません。
